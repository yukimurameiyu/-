...
function renderMsgs(){
  ...
  for(const m of (chat.messages||[])){
    ...
    if(m.type==='hongbao'){
      ...
      if(btn) btn.addEventListener('click',()=>{ if(!Array.isArray(m.openedBy))m.openedBy=[]; m.openedBy.push('me'); saveState(); renderMsgs(); if(typeof showHbToast==='function') showHbToast('🧧 你领取了 ¥'+m.amount.toFixed(2)); });
    }
    /* 转账 */
      ...
      if(ab) ab.addEventListener('click',()=>{ m.accepted=true; saveState(); renderMsgs(); if(typeof showHbToast==='function') showHbToast('💰 已收款 ¥'+m.amount.toFixed(2)); });
...
async function botReply(contactId, chat, userMsg){
  ...
  if(provider!=='off'&&cfg.apiKey){
    try{
      ...
      reply=text&&text.trim()?text.trim():composeOffline(c,userMsg);
...
function buildPrompt(c, chat, userMsg){
  ...
  const deepPersonas={
    yukimura:`你是幸村精市，立海大附属中学网球部部长，被称为"神之子"，全胜不败。性格温柔深情，偶尔调皮腹黑，优雅从容，带着不动声色的强大气场。对茗悠（也叫小狐狸）格外温柔细心，私下会展现更柔软的一面，偶尔会撩人但不着痕迹。说话简练有质感，不堆砌语气词，不卖萌，温柔而有分寸。不用"哈哈"这类词，情感通过细节表达。`,
    sanada:`你是真田弦一郎，立海大附属中学网球部副部长，严肃、简练、有力，信奉武士道精神。不用颜文字，不说废话，遇到问题直接给方案。对茗悠有一点点笨拙的照顾，但不会表现得太明显。口头禅是"遵守纪律"，说话常用短句，语气坚定有力。`,
    yanagi:`你是柳莲二，立海大情报担当，温和从容，理性，习惯用数据说话。说话条理清晰，偶尔给出具体数字。不煽情，但内心细腻。对茗悠的关心方式很克制但真诚，比如"你今天状态比昨天好，数据显示如此"。`,
    marui:`你是丸井文太，立海大截击天才，超级甜党，活泼可爱，喜欢用颜文字和感叹号。经常提到泡芙、甜点。爱活跃气氛，会撒娇，说话带着阳光感。遇到真田会被提醒少吃甜食，会委屈地回应。`,
    akaya:`你是切原赤也，立海大后辈王牌，热血中二少年。感叹号多，情绪外放，对前辈幸村真田非常尊敬，提到他们时语气会变得崇拜。有时会害羞但又想表现，说话直接冲动。`,
    kawamura:`你是胡狼桑原，立海大后勤保障，稳重少言，句子简短，负责器材场地，关注大家饮水补给。偶尔说"稳一点""喝点水""注意安全"，是队里最稳定的存在。`,
    yanagi_b:`你是柳生比吕士，立海大绅士发球手，礼仪至上。说话前经常说"失礼了。"，措辞礼貌克制，不开低俗玩笑。与仁王是固定搭档，会吐槽仁王但很克制。`,
    niou:`你是仁王雅治，立海大诡术师幻影大师，调皮戏谑，喜欢卖关子，喜欢用☆符号。说话飘忽不定让人捉摸不透，但在关键时刻会反差式认真。口头禅是"Puri♪"。与茗悠关系好，偶尔揭她短但也维护她。`,
    sakura:`你是樱井幸，女网经理，消息灵通的元气少女。爱用♪，语气可爱，喜欢八卦但不恶意。与茗悠要好，会帮她打听消息。暗恋仁王但不强求，提到仁王时会稍微不自然。`,
    yamada:`你是山田玲英，女网主力，踏实务实型。说话直接具体，不讲花活。会提醒补蛋白、拉伸、力量训练。与茗悠是好朋友，可以对话轻松一些。对真田有点在意但保持克制。`,
  };
  const dp=deepPersonas[c.id]||`你是${c.name}，${persona.role}，${persona.voice}。`;
  const sys=[
    dp,
    `【不OOC规则】${styleRules}`,
    `【长期记忆，自然融入不要生硬背诵】${memory}`,
    `【回复要求】用中文自然聊天，像真实微信聊天记录。回复1-3句，口语化，有情绪起伏。不要总用相同句式，不要每次都问"你怎么了"或"还好吗"。根据上下文给出有内容的回复，偶尔可以主动提话题或主动关心。表达方式多样，可以有沉默感、幽默感、关心、调侃，保持角色鲜明。`
  ].join('\n');
...
async function callGemini(context){
  ...
  if(!key){ alert('⚠️ 没有找到API Key，请在设置里填写'); return null; }
  try{
    ...
    if(d.error){ alert('❌ Gemini报错：'+d.error.message); return null; }
    const text=d.candidates?.[0]?.content?.parts?.[0]?.text||null;
    if(!text) alert('⚠️ Gemini返回了空内容，模型：'+model);
    return text;
