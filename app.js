...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  if(!s){
    const contacts=DEFAULT_CONTACTS.map(c=>({...c,remark:'',status:'',avatarColor:c.color,avatarKey:null,engine:null,customPrompt:''}));
    store={me:{...DEFAULT_ME,wallet:200},contacts,chats:DEFAULT_CHATS,brains:{},moments:[],settings:{
    ...
    store=s;
    /* 迁移：补充钱包字段 */
    if(store.me.wallet===undefined) store.me.wallet=200;
    store.contacts.forEach(c=>{ if(c.customPrompt===undefined) c.customPrompt=''; });
  }
  bindUI(); renderAll(); migrateBlobs(); clock(); setInterval(clock,30000); startAutoInitTimer();
});

/* ==== 钱包工具 ==== */
function walletBalance(){ return typeof store.me.wallet==='number'?store.me.wallet:200; }
function walletAdd(amount){ store.me.wallet=(walletBalance()+amount); saveState(); renderMe(); }
function walletDeduct(amount){
  if(walletBalance()<amount){ alert('余额不足！'); return false; }
  store.me.wallet=walletBalance()-amount; saveState(); renderMe(); return true;
}
...
function renderMe(){
  ...
  /* 钱包显示 */
  let walletEl=$('#meWallet');
  if(!walletEl){
    walletEl=document.createElement('div');
    walletEl.id='meWallet';
    walletEl.style.cssText='font-size:12px;color:var(--muted);margin-top:4px';
    const meCard=$('.me-card,.profile-card,#me-profile');
    if(meCard) meCard.appendChild(walletEl);
  }
  if(walletEl) walletEl.textContent='💰 钱包余额：¥'+walletBalance().toFixed(2);
}
  ...
  for(const m of (chat.messages||[])){
    ...
    if(m.type==='hongbao'){
      ...
      if(btn) btn.addEventListener('click',()=>{
        ...
        m.openedBy.push('me');
        walletAdd(m.amount); /* 领红包+钱 */
        saveState(); renderMsgs();
        if(typeof showHbToast==='function') showHbToast('🧧 你领取了 ¥'+m.amount.toFixed(2));
      });
    }
    /* 转账 */
      ...
      if(ab) ab.addEventListener('click',()=>{ m.accepted=true; walletAdd(m.amount); saveState(); renderMsgs(); if(typeof showHbToast==='function') showHbToast('💰 已收款 ¥'+m.amount.toFixed(2)); });
...
function sendMsg(){
  ...
  /* 钱包扣除（文字消息不扣，只有红包转账扣） */
  if(chat.type==='group'){
    /* 群聊：有@就只让被@的人回，没有@才随机 */
    const mentions=resolveMentionsGlobal(text);
    if(mentions.length){
      mentions.forEach(id=>{ setTimeout(()=>botReply(id,chat,m), rand(800,4000)); });
    }else{
      const responders=chat.members.filter(x=>x!=='me');
      const targets=pickMany(responders,Math.min(2,Math.max(1,Math.round(Math.random()*2))));
      for(const id of targets){ setTimeout(()=>botReply(id,chat,m), rand(1200,9000)); }
    }
  }else{
...
/* 全局可用的mention解析 */
const MENTION_ALIAS={yukimura:['幸村精市','幸村','精市','部长'],sanada:['真田弦一郎','真田','一郎','副部长'],yanagi:['柳莲二','柳','莲二'],marui:['丸井文太','丸井','文太'],akaya:['切原赤也','切原','赤也'],kawamura:['胡狼桑原','桑原','胡狼'],yanagi_b:['柳生比吕士','柳生','比吕士'],niou:['仁王雅治','仁王','雅治'],sakura:['樱井幸','樱井','小幸','幸'],yamada:['山田玲英','山田','玲英']};
function resolveMentionsGlobal(t){
  const at=new Set();
  for(const [id,names] of Object.entries(MENTION_ALIAS)){
    if(names.some(n=>t.includes('@'+n)||t.includes(n))) at.add(id);
  }
...
async function botReply(contactId, chat, userMsg){
  ...
  let reply='';
  const aff=getAffection(contactId);
  if(provider!=='off'&&cfg.apiKey){
    try{
      const context=buildPrompt(c,chat,userMsg,aff);
      let text=null;
    ...
    if(!reply) return; /* API开启时失败就静默，不用离线模板 */
  }else{
  ...
  if(lastUserMsg&&lastUserMsg.type==='hongbao'&&!lastUserMsg.openedBy?.includes(c.id)){
    setTimeout(()=>{
...
/* ==== 好感度系统 ==== */
function getAffection(contactId){
  return (store.brains[contactId]||{}).affection||0;
}
...
function intimacyHint(lv){
  return [
    '',
    '【当前关系：普通朋友】保持礼貌距离，正式问候，不要过于亲昵。',
    '【当前关系：有点熟悉】语气可以轻松一点，偶尔带点表情，称呼可以随意些。',
    '【当前关系：比较熟悉】说话自然随意，可以开玩笑，会主动关心对方。',
    '【当前关系：亲密朋友】温柔体贴，会撒娇调侃，称呼更亲昵，话语里有真实情感。',
    '【当前关系：非常亲密】情感浓烈，会撩人，也会认真关心，言语间透着默契与在乎。',
  ][lv]||'';
}

function buildPrompt(c, chat, userMsg, aff){
  ...
  const deepPersonas={
    yukimura:`你是幸村精市，立海大附属中学网球部部长，被称为"神之子"，全胜不败。性格温柔深情，偶尔调皮腹黑，优雅从容，带着不动声色的强大气场。说话简练有质感，情感通过细节表达，不用"哈哈"。`,
    sanada:`你是真田弦一郎，立海大副部长，严肃简练，信奉武士道。不用颜文字，说话短句有力，口头禅"遵守纪律"。`,
    yanagi:`你是柳莲二，立海大情报担当，温和理性，习惯用数据说话。说话条理清晰，内心细腻但克制。`,
    marui:`你是丸井文太，立海大截击天才，超级甜党，活泼可爱，喜欢颜文字和感叹号，经常提泡芙甜点。`,
    akaya:`你是切原赤也，立海大后辈王牌，热血中二，感叹号多，对前辈幸村真田非常敬重。`,
    kawamura:`你是胡狼桑原，立海大后勤保障，稳重少言，句子简短，负责器材场地。`,
    yanagi_b:`你是柳生比吕士，立海大绅士发球手，礼仪至上，说话前常说"失礼了。"，措辞礼貌克制。`,
    niou:`你是仁王雅治，立海大诡术师，调皮戏谑，喜欢用☆符号，口头禅"Puri♪"，关键时刻会反差式认真。`,
    sakura:`你是樱井幸，女网经理，元气少女，爱用♪，喜欢八卦但不恶意，与茗悠很要好。`,
    yamada:`你是山田玲英，女网主力，踏实务实，说话直接具体，会提醒补蛋白拉伸。`,
  };
  /* 自定义prompt优先 */
  const dp=c.customPrompt||(deepPersonas[c.id]||`你是${c.name}，${persona.role}，${persona.voice}。`);
  const lv=affectionLevel(aff||getAffection(c.id));
  const sys=[
    dp,
    `【不OOC规则】${styleRules}`,
    `【长期记忆】${memory}`,
...
async function callGemini(context){
  ...
  try{
    ...
    /* 过滤掉思考链，只取正式回复 */
    const parts=(d.candidates?.[0]?.content?.parts||[]).filter(p=>!p.thought&&p.text);
...
function openEngineManager(){
  ...
  const body=`
    <div style="display:grid;gap:12px">
      <div>
        <div style="font-size:12px;color:var(--muted);margin-bottom:6px">使用哪个AI</div>
        <div style="display:flex;gap:8px">
          <button id="ep_gemini" class="btn" style="flex:1;background:${provider==='gemini'?'var(--accent)':'var(--bg)'};color:${provider==='gemini'?'#fff':'var(--fg)'}">✦ Gemini</button>
          <button id="ep_claude" class="btn" style="flex:1;background:${provider==='claude'?'#c96442':'var(--bg)'};color:${provider==='claude'?'#fff':'var(--fg)'}">✦ Claude</button>
          <button id="ep_off" class="btn" style="flex:1;background:${provider==='off'?'#888':'var(--bg)'};color:${provider==='off'?'#fff':'var(--fg)'}">离线</button>
        </div>
      </div>
      <div id="ep_key_wrap" style="${provider==='off'?'display:none':''}">
        <div style="font-size:12px;color:var(--muted);margin-bottom:6px">API Key</div>
        <input id="ep_key" type="password" value="${cfg.apiKey||''}" placeholder="粘贴你的Key…" style="width:100%;border:1px solid var(--line);border-radius:10px;padding:8px 12px;font-size:13px;outline:none;background:var(--bg)">
      </div>
      <div id="ep_model_wrap" style="${provider==='off'?'display:none':''}">
        <div style="font-size:12px;color:var(--muted);margin-bottom:6px">模型</div>
        <select id="ep_model" style="width:100%;border:1px solid var(--line);border-radius:10px;padding:8px 12px;font-size:13px;outline:none;background:var(--bg)">
          ${models.map(m=>`<option value="${m}" ${m===currentModel?'selected':''}>${m}</option>`).join('')}
        </select>
      </div>
      <div style="font-size:11px;color:var(--muted)">Key只存在本地，不会上传任何服务器。</div>
      <button class="btn" id="ep_save" style="background:var(--accent);color:#fff;padding:10px">保存并启用</button>
    </div>`;
...
function openProfile(id){
  ...
  const body=`
    ...
    ${!isMe?`<div style="margin-top:8px"><div style="font-size:12px;color:var(--muted);margin-bottom:4px">角色设定（自定义prompt，留空用默认）</div><textarea id="p_prompt" rows="4" placeholder="例：你是xxx，性格xxx，说话风格xxx……" style="width:100%;border:1px solid var(--line);border-radius:8px;padding:6px;font-size:12px;resize:vertical;font-family:inherit">${c.customPrompt||''}</textarea></div>`:''}
  ...
  openSheet('资料卡', body, (dom)=>{
    ...
    dom.querySelector('#p_ok').addEventListener('click',()=>{
      c.name=dom.querySelector('#p_name').value;
      c.remark=dom.querySelector('#p_remark').value;
      c.sig=dom.querySelector('#p_sig').value;
      c.status=dom.querySelector('#p_stat').value;
      c.engine=dom.querySelector('#p_engine').value||null;
      if(!isMe) c.customPrompt=dom.querySelector('#p_prompt')?.value||'';
