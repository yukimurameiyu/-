...
const MENTION_ALIAS={yukimura:['幸村精市','幸村','精市','部长'],sanada:['真田弦一郎','真田','副部长'],yanagi:['柳莲二','柳','莲二'],marui:['丸井文太','丸井','文太'],akaya:['切原赤也','切原','赤也'],kawamura:['胡狼桑原','桑原'],yanagi_b:['柳生比吕士','柳生'],niou:['仁王雅治','仁王','雅治'],sakura:['樱井幸','樱井'],yamada:['山田玲英','山田']};
function resolveMentionsGlobal(t){const at=new Set();for(const [id,names] of Object.entries(MENTION_ALIAS)){if(names.some(n=>t.includes('@'+n)||t.includes(n)))at.add(id);}return Array.from(at);}
...
async function botReply(contactId, chat, userMsg){
  ...
  if(provider!=='off'&&cfg.apiKey){
    ...
    if(!reply) return; /* API开启失败静默，不用离线模板 */
  }else{
...
function openProfile(id){
  ...
  const body=`
    ...
    ${!isMe?`<div style="margin-top:8px"><div style="font-size:12px;color:var(--muted);margin-bottom:3px">角色设定（自定义prompt，留空用默认）</div><textarea id="p_prompt" rows="3" placeholder="你是xxx，性格xxx，说话风格xxx…" style="width:100%;border:1px solid var(--line);border-radius:8px;padding:6px;font-size:12px;font-family:inherit;resize:vertical">${c.customPrompt||''}</textarea></div>`:''}
